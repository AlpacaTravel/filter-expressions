...
const evaluate = (expression, target = null, options = {}) => {
  ...
  if (
    modifier.test(resolvedExpressions[1]) &&
    typeof resolvedExpressions[1] === "string" &&
    options &&
    options.modifiers
  ) {
  ...
  switch (resolvedExpressions[0].toLowerCase()) {
    ...
    case "in": {
      if (Array.isArray(comparative)) {
        return comparative.reduce(
          (carry, comp) => carry || expression.slice(2).indexOf(comp) > -1,
          false
        );
      }
    ...
    case "all": {
      return resolvedExpressions
        .slice(1)
        .reduce((carry, expr) => carry && expr === true, true);
    }
    case "any": {
      return resolvedExpressions
        .slice(1)
        .reduce((carry, expr) => carry || expr === true, false);
    }
    case "none": {
      return !resolvedExpressions
        .slice(1)
    ...
    case "get": {
      const t = target
        ? _get(target, resolvedExpressions[1], resolvedExpressions[2])
        : resolvedExpressions[1];
      return t;
    }
